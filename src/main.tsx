...
import './index.css'
import { initInspectRuntime } from './inspect-runtime'
...
/**
 * Check if a file exports a compound component (multiple related components)
 */
function isCompoundComponent(exports: string[]): boolean {
  // If file has 3+ capitalized exports, it's likely a compound component
  const componentExports = exports.filter(name =>
    name !== '__preview' &&
    name !== 'default' &&
    name[0] === name[0].toUpperCase()
  )
  return componentExports.length >= 3
}

/**
 * Get smart default props for a component based on its name
 */
function getSmartDefaults(componentName: string, componentPath: string): {
  ...
  if (name.includes('accordion') || name.includes('collapsible')) {
    return {
      props: { defaultValue: 'item-1', type: 'single', collapsible: true },
  ...
  // Card, Panel - give reasonable width and sample children
  if (name.includes('card') || name.includes('panel')) {
    return {
      props: { children: React.createElement('div', { className: 'p-6' },
        React.createElement('h3', { className: 'text-lg font-semibold' }, 'Card Title'),
        React.createElement('p', { className: 'text-gray-600 mt-2' }, 'Card content goes here.')
      )},
      wrapperStyle: { width: '400px' }
    }
  }

  // Label - needs children
  ...
  // Button, Link, Badge - need children
  ...
  // Select - compound component, mark as such
...
/**
 * Preview for compound components - shows all exports from the file
 */
function CompoundComponentPreview({
  exports,
  componentPath
}: {
  exports: Array<{ name: string; component: ComponentType<any> }>
  componentPath: string
}) {
  return (
    <div className="min-h-screen w-full bg-gray-50">
      ...
      <div className="bg-white border-b px-4 py-2 text-sm text-gray-600 flex items-center gap-2">
        ...
        <code className="bg-gray-100 px-2 py-0.5 rounded text-xs">{componentPath}</code>
        <span className="ml-2 text-xs text-gray-500">({exports.length} exports)</span>
      </div>

      {/* Grid of all exported components */}
      <div className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {exports.map(({ name, component: Component }) => {
            const defaults = getSmartDefaults(name, componentPath)
            return (
              <div key={name} className="bg-white rounded-lg border shadow-sm overflow-hidden">
                ...
                <div className="p-4 min-h-[100px] flex items-center justify-center">
                  <ComponentErrorBoundary componentPath={`${componentPath}#${name}`}>
        ...
        <div className="mt-6 text-center text-sm text-gray-500">
          <p>This file exports multiple components that are typically used together.</p>
          <p className="mt-1">View App.tsx to see how they're composed.</p>
        </div>
      </div>
    </div>
  )
  ...
  if (componentPath) {
    ...
    try {
      ...
      if (modules[modulePath]) {
        ...
        console.log('[Component Preview] Module exports:', exportNames)
        ...
        // Check if this is a compound component (multiple exports)
        if (isCompoundComponent(exportNames)) {
          console.log('[Component Preview] Detected compound component')

          // Collect all component exports
          const componentExports: Array<{ name: string; component: ComponentType<any> }> = []
          for (const [name, exp] of Object.entries(mod)) {
            if (
              ...
              componentExports.push({ name, component: exp as ComponentType<any> })
            }
          }
        ...
        // Single component mode - prefer default export, fall back to first capitalized named export
